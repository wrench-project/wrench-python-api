...
if __name__ == "__main__":
    ...
    try:
        ...
        task1 = workflow.add_task(workflow.name, "task1", 10000000000.0, 1, 1, 0)
        print(f"{task1}")
        job = simulation.create_standard_job(workflow.name, [task1], {})
        print(f"{job}")
        ...
        task2 = workflow.add_task(workflow.name, "task2", 10000000000.0, 1, 1, 0)
        job = simulation.create_standard_job(workflow.name, [task2], {})
        vm_cs.submit_standard_job(job)
        print(f"Simulation, time is {simulation.get_simulated_time()}")
        print(f"Waiting for job completion...")
        event = simulation.wait_for_next_event()
