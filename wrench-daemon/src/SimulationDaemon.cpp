...
void SimulationDaemon::getTime(const Request &req, Response &res) {
    ...
    auto time = simulation_thread_state->simulation_controller->getSimulationTime();
...
void SimulationDaemon::getAllHostnames(const Request &req, Response &res) {
    ...
    std::vector<std::string> hostnames = simulation_thread_state->simulation_controller->getAllHostnames();
...
void SimulationDaemon::terminateSimulation(const Request &req, Response &res) {
    ...
    simulation_thread_state->simulation_controller->stopSimulation();
...
void SimulationDaemon::addTime(const Request &req, Response &res) {
    ...
    simulation_thread_state->simulation_controller->advanceSimulationTime(time_to_sleep);
...
void SimulationDaemon::getSimulationEvents(const Request &req, Response &res) {
    ...
    simulation_thread_state->simulation_controller->getSimulationEvents(events);
...
void SimulationDaemon::waitForNextSimulationEvent(const Request &req, Response &res) {
    ...
    auto event = simulation_thread_state->simulation_controller->waitForNextSimulationEvent();
...
void SimulationDaemon::addService(const Request &req, Response &res) {
    ...
    json answer;
    try {
        std::string service_name = simulation_thread_state->simulation_controller->addService(req_body);
        answer["success"] = true;
...
void SimulationDaemon::submitStandardJob(const Request &req, Response &res) {
    ...
    json answer;
    try {
        simulation_thread_state->simulation_controller->submitStandardJob(req_body);
        answer["success"] = true;
    } catch (std::exception &e) {
        answer["success"] = false;
        answer["failure_cause"] = e.what();
...
void SimulationDaemon::createStandardJob(const Request &req, Response &res) {
    ...
    json answer;
    try {
        std::string jobID = simulation_thread_state->simulation_controller->createStandardJob(req_body);
        answer["success"] = true;
        answer["job_id"] = jobID;
    } catch (std::exception &e) {
        answer["success"] = false;
